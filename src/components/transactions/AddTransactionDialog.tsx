...
export function AddTransactionDialog({ categories, initialData, trigger }: { categories: Category[], initialData?: Transaction, trigger?: React.ReactNode }) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[420px] rounded-2xl border-border/50 p-0 overflow-hidden">
        <div className="p-6 pb-0">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold">{initialData ? "Editar Movimiento" : "Nuevo Movimiento"}</DialogTitle>
          </DialogHeader>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 pt-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label htmlFor="date" className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Fecha</Label>
              <Input 
                className="h-11 rounded-xl bg-muted/30 border-border/50 focus:bg-background" 
                id="date" 
                type="date" 
